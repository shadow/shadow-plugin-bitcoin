...
void __shadow_plugin_init__(ShadowFunctionTable* shadowlibFuncs) {
	 ...
	 */
	int success = shadowlib.registerPlugin(&bitcoincliplugin_new, &bitcoincliplugin_free, &bitcoincliplugin_ready);

	/* we log through Shadow by using the log function it supplied to us */
	if(success) {
		shadowlib.log(SHADOW_LOG_LEVEL_MESSAGE, __FUNCTION__,
				"successfully registered bitcoincli plug-in state");
	} else {
		shadowlib.log(SHADOW_LOG_LEVEL_CRITICAL, __FUNCTION__,
				"error registering bitcoincli plug-in state");
	}
}
