...
#define ACTIVE_PLUGIN PLUGIN_NETMINE_LOGSERVER
...
void *_logserver_new(struct args_t *args) {
	bitcoindpreload_setPluginContext(ACTIVE_PLUGIN);
	_plugin_ctors();
	assert(args);
	int argc = args->argc;
...
void _logserver_new_void(struct args_t *args) {
	_logserver_new(args);
}
...
static void plugin_new(int argc, char* argv[]) {
	...
	assert(rc);
	rc = pth_uctx_make(plugin_context, NULL, 130000, NULL, _logserver_new_void, &args, NULL);
	assert(rc);

	//pth_t t = pth_spawn(PTH_ATTR_DEFAULT, (void *(*)(void*))&_logserver_new, &args);
	bitcoindpreload_setPluginContext(ACTIVE_PLUGIN);
...
void __shadow_plugin_init__(ShadowFunctionTable* shadowlibFuncs) {
	...
	if(success) {
		shadowlib.log(SHADOW_LOG_LEVEL_MESSAGE, __FUNCTION__,
				"successfully registered logserver plug-in state");
	} else {
		shadowlib.log(SHADOW_LOG_LEVEL_CRITICAL, __FUNCTION__,
				"error registering logserver plug-in state");
	}
}
...
const gchar* g_module_check_init(GModule *module) {
	fprintf(stderr, "gmodule init logserver\n");
